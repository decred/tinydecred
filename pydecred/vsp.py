...
class VotingServiceProvider(object):
    ...
    @staticmethod
    def getAgendasInfo(net):
        """
        The agendas info that are used for voting.

        Args:
            dcrdata (object): a connection to dcrdata

        Returns:
            AgendasInfo: the current agendas.
        """
        network = "testnet" if net.Name == "testnet3" else net.Name
        res = tinyhttp.get("https://%s.dcrdata.org/api/stake/vote/info" % network)
        return AgendasInfo.parse(res)
